...
const PostShcema = new Schema<IPost, IPostModel>({
    title: {
        type: String,
        required: true,
        trim: true
...
PostShcema.method('upvote', async function(this: IPost, userId: string) {
...
PostShcema.method('downvote', async function(this: IPost, userId: string) {
    const userIdObject = new Types.ObjectId(userId)
...
const Post = model<IPost, IPostModel>('Post', PostShcema)
